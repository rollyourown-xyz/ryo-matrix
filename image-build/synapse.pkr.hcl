...
# Specify the synapse version to use in the image build
variable "synapse_version" {
  description = "Mandatory: The synapse version to use in the image build."
  type        = string
}
...
locals {
  ...
  build_playbook_file  = "${abspath(path.root)}/playbooks/provision-synapse.yml"
  build_extra_vars     = "host_id=${var.host_id} synapse_version=${var.synapse_version}"
}
