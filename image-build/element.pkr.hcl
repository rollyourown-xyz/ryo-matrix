...
#---------------------------------------------------------------------------
# Packer template for building LXD container image for Element-Web webserver
...
# Specify the element version to use in the image build
variable "element_version" {
  description = "Mandatory: The element-web version to use in the image build."
  type        = string
}
...
locals {
  ...
  build_playbook_file  = "${abspath(path.root)}/playbooks/provision-element.yml"
  build_extra_vars     = "host_id=${var.host_id} element_version=${var.element_version}"
}
